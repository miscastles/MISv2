...
namespace MIS
{
    public partial class frmTerminalType : Form
    {
        ...
        private void LoadTerminalType()
        {
            ...
            ClearListView();            
            dbAPI.ExecuteAPI("GET", "View", "", "", "Terminal Type", "", "ViewTerminalType");

            if (!clsGlobalVariables.isAPIResponseOK) return;

            if (clsTerminalType.RecordFound)
            {
                while (clsArray.TerminalTypeID.Length > i)
                {
                    //clsProductInfo.ResetProductInfo();
                    clsTerminalType.ClassTerminalTypeID = int.Parse(clsArray.TerminalTypeID[i].ToString());
                    clsTerminalType.ClassDescription = clsArray.Description[i];
        ...
        private void AddItem(int inLineNo)
        {
            ...
            item.SubItems.Add(clsTerminalType.ClassTerminalTypeID.ToString());
            item.SubItems.Add(clsTerminalType.ClassDescription.ToString());
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (!fEdit)
            {
                ...
                MessageBox.Show("New Terminal Type successfully saved", "Saved",
            ...
            else
            {
                ...
                dbAPI.ExecuteAPI("PUT", "Update", "Terminal Type", clsSearch.ClassAdvanceSearchValue, "", "", "UpdateCollectionDetail");

                MessageBox.Show("Terminal Type has been successfully modified", "Edited",
                MessageBoxButtons.OK,
                MessageBoxIcon.Information,
                MessageBoxDefaultButton.Button1);
            }
        ...
        private void frmTerminalType_Load(object sender, EventArgs e)
        {
            dbAPI = new clsAPI();
        ...
        private void frmTerminalType_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Escape:
