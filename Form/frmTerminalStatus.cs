...
namespace MIS
{
    public partial class frmTerminalStatus : Form
    {
        ...
        private void LoadTerminalStatus()
        {
            ...
            ClearListView();
            dbAPI.ExecuteAPI("GET", "View", "Enrollment", "", "Terminal Status", "", "ViewTerminalStatus");

            if (!clsGlobalVariables.isAPIResponseOK) return;

            if (clsTerminalStatus.RecordFound)
            {
                while (clsArray.TerminalStatusID.Length > i)
                {
                    //clsProductInfo.ResetProductInfo();
                    clsTerminalStatus.ClassTerminalStatusID = int.Parse(clsArray.TerminalStatusID[i].ToString());
                    clsTerminalStatus.ClassDescription = clsArray.Description[i];
        ...
        private void AddItem(int inLineNo)
        {
            ...
            item.SubItems.Add(clsTerminalStatus.ClassTerminalStatusID.ToString());
            item.SubItems.Add(clsTerminalStatus.ClassDescription.ToString());
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (!fEdit)
            {
                ...
                dbAPI.ExecuteAPI("POST", "Insert", "", "", "Terminal Status", sSQL, "InsertMaintenanceMaster");
            ...
            else
            {
                ...
                Debug.WriteLine("UpdateTerminalStatus::" + "clsSearch.ClassAdvanceSearchValue=" + clsSearch.ClassAdvanceSearchValue);

                dbAPI.ExecuteAPI("PUT", "Update", "Terminal Status", clsSearch.ClassAdvanceSearchValue, "", "", "UpdateCollectionDetail");

                MessageBox.Show("Terminal Status has been successfully modified", "Edited",
                MessageBoxButtons.OK,
                MessageBoxIcon.Information,
                MessageBoxDefaultButton.Button1);
            }
        ...
        private void frmTerminalStatus_Load(object sender, EventArgs e)
        {
            dbAPI = new clsAPI();
        ...
        private void frmTerminalStatus_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Escape:
